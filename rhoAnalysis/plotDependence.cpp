...
int main( int argc, char** argv){
	...
	if( inBeam == 0 || inBeam == 10.4){
		...
		if( applyCorr > 4 ){
			rChain->Add( base + "final_skims/rho_skims/rotated_10.4_acc.root");
		}
	}
	if( inBeam == 0 || inBeam == 10.6){
		...
		if( applyCorr > 4 ){
			rChain->Add( base + "final_skims/rho_skims/rotated_10.6_acc.root");
		}
	}
	...
	if( applyCorr > 4 ){
		while (reader_r.Next()) {
			...
			for( int i = 0; i < (int) ( r.end() - r.begin() ); i++ ){
				...
				weight = 1;// 0.5*rhoWeight_sym[i];// - (double)(i==1);
				eventWeightErr = 0;
				...
				if( *Mx_2pi > Mx_min && *Mx_2pi < Mx_max ){
					events_in_bin[3][(int)(!chargeIdx)][this_bin_Q2][this_bin_xB][kaon_bin_Z][p_bin][this_bin_E]++;
					weights_in_bin[3][(int)(!chargeIdx)][this_bin_Q2][this_bin_xB][this_bin_Z]+= weight;
					//events_in_bin[3][chargeIdx][this_bin_Q2][this_bin_xB][kaon_bin_Z][p_bin]++;
					//hZ_r_bac[this_bin_Q2][this_bin_xB][chargeIdx]->Fill( r[i].getZ(), weight );
				}			
			}
		...
		for( int j = 1; j <= bins_xB; j++ ){
			...
			for( int k = 0; k < bins_Z; k++ ){
				...
				if( applyCorr <= 3){
					...
					setBin( hZ_r[i-1][j-1][0],  k, events_in_bin[2][0][i-1][j-1], weights_in_bin[2][0][i-1][j-1][k], corrector, 4, matchType, 0);
					setBin( hZ_r[i-1][j-1][1],  k, events_in_bin[2][1][i-1][j-1], weights_in_bin[2][1][i-1][j-1][k], corrector, 4, matchType, 1);


					setBin( hZ_r_bac[i-1][j-1][0],  k, events_in_bin[3][0][i-1][j-1], weights_in_bin[3][0][i-1][j-1][k], corrector, 4, matchType, 0);
					setBin( hZ_r_bac[i-1][j-1][1],  k, events_in_bin[3][1][i-1][j-1], weights_in_bin[3][1][i-1][j-1][k], corrector, 4, matchType,1);
			...
			if( applyCorr > 4 ){
				...
				if( hZ_r_bac[i-1][j-1][0]->Integral()>0){
				hZ_r_bac[i-1][j-1][0]->Print("All");}
