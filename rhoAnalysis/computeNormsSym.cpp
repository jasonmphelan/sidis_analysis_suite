...
int main( int argc, char** argv){
	...
	double beam_energy = atof(argv[3]);
	...
	TH2F* hMx_mrho = new TH2F("hMx_rho", "", 200, 0, 5.5, 200, 0, 2);
	TH3F * hNorms[2];
	...
	for( int x = 0; x < nBinsXb; x++ ){
		for( int q = 0; q < nBinsQ2; q++ ){
			for( int z = 0; z <  nBinsZ; z++ ){
				...
				hMx_2pi[0][x][q][z] = new TH1F( Form("hMx_pip_%i_%i_%i", x, q, z), "pip " + binLims, 200, 0, 5.5);
				hMx_2pi[1][x][q][z] = new TH1F( Form("hMx_pim_%i_%i_%i", x, q, z), "pim " + binLims, 200, 0, 5.5);
	...
	if(beam_energy == 10.2 || beam_energy == 0 )file_rec->Add("../trees/final_skims/rho_skims/rotated_10.2" + in_name + ".root");
	if(beam_energy == 10.4 || beam_energy == 0 )file_rec->Add("../trees/final_skims/rho_skims/rotated_10.4" + in_name + ".root");
	if(beam_energy == 10.6 || beam_energy == 0 )file_rec->Add("../trees/final_skims/rho_skims/rotated_10.6" + in_name + ".root");
	...
	while (reader_rec.Next()) {
		...
		for( int i = 0; i < (int)(pi.end() - pi.begin());i++ ){
			...
			hMx_2pi[chargeIdx][this_bin_xB][this_bin_Q2][this_bin_Z]->Fill( (*Mx_2pi)*(*Mx_2pi) , weight );//  weight );
			//hMx_2pi[(int)(!chargeIdx)][this_bin_xB][this_bin_Q2][this_bin_Z]->Fill( (*Mx_2pi) , 0.5*rhoWeight_sym[i] );

			if( *Mx_2pi > bounds->X() && *Mx_2pi < bounds->Y() ) hM_rho_back[chargeIdx][this_bin_xB][this_bin_Q2][this_bin_Z]->Fill( (*M_rho) , weight );
			if( *Mx_2pi < bounds->X() ) hM_rho[chargeIdx][this_bin_xB][this_bin_Q2][this_bin_Z]->Fill( (*M_rho) , weight );

			/*
			if( rhoWeight_sym[i] <= 0 || rhoWeight_sym[i] > 100 ){ continue; }
			weight =  corrector.getCorrectionFactor( 2, (int)(!chargeIdx) );

			if( *Mx_2pi > bounds->X() && *Mx_2pi < bounds->Y() ) hM_rho_back[(int)(!chargeIdx)][this_bin_xB][this_bin_Q2][this_bin_Z]->Fill( (*M_rho) , 0.5*rhoWeight_sym[i]*weight );
			if( *Mx_2pi < bounds->X() ) hM_rho[(int)(!chargeIdx)][this_bin_xB][this_bin_Q2][this_bin_Z]->Fill( (*M_rho) , 0.5*rhoWeight_sym[i]*weight);
			*/
		}
	}
