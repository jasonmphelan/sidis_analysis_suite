...
:
	...
	std::vector<TLorentzVector> parent_p;
	...
	if( singleFile == 1 ){
		...
		if( RunType == 1 ){
			...
			outTree->Branch("parent_p", &parent_p);			
	...
	////////////////////////////////////Begin file loop////////////////////////////////////////////////////
		...
		int NeventsTotal = c12.getReader().getEntries();       
			...
			if(RunType == 1 && inclusive != 1){	
				...
				bool foundPip, foundPim, foundP, foundEl; 
				foundPip = foundPim = foundP = foundEl = false;
				double cumMom = 0;
				for ( int i = 0; i<nMcPart; i++ ){
					//cout<<c12.mcparts()->getPid(i)<<std::endl;
					...
					int pId = mcId;//c12.mcparts()->getPid(c12.mcparts()->getParent(i) - 1);
					//if( abs(pId) ==91 || abs(pId) == 92 || abs(pId) == 1 || abs(pId) == 2){
					if(i >= nMcPart-4){
						cumMom+=c12.mcparts()->getPz(i);
						if( mcId == 211) foundPip=true;
						if( mcId == -211 ) foundPim = true;
						if( mcId == 2212 ) foundP = true;
						if( mcId == 11 ) foundEl = true;
						
						//if( foundPip && foundPim && foundEl && foundP ) isDIS = false;
						if( abs(cumMom -10.2) < .005 )isDIS = false;
					}
				}
				//if( abs(cumMom -10.2) < .005 )cout<<"Cumulative Momentum : "<<cumMom<<std::endl;
			...
			bool good_dat_e = false;
			bool good_sim_e = false;
			//if( !anal.applyElectronDetectorCuts( e )){continue;}
			if( (e.getQ2() > 2 && e.getQ2() < 8 ) && ( e.getXb() > .1 && e.getXb() < .64 )) good_dat_e = true;
			...
			if( RunType == 1 ){
				...
				if( (e_gen.getQ2() > 2 && e_gen.getQ2() < 8 ) && ( e_gen.getXb() > .1 && e_gen.getXb() < .64 )) good_sim_e = true;
			}

			////////////////Pion analysis/////////////////
			if(!good_dat_e && !good_sim_e)continue;
			
			pion pi_dummy;
			genPion genPi_dummy;
			//Find good pions			
			for(int i = 0; i < Npi; i++){
				bool good_dat_pi = false;
				bool good_sim_pi = false;
				pi_dummy.Clear();
				pi_dummy.setMCPion( (bool)RunType );
				pi_dummy.setPion( e.getQ(),e.get4Momentum(), pions[i] );
				if( anal.applyPionDetectorChi2( pi_dummy ) && pi_dummy.getZ() > .3 && anal.applyAcceptanceMatching(pi_dummy, 2)) good_dat_pi=true;
				...
				if( RunType == 1 ){
					...
					if( pi_match > -1  ){ //&& abs( mcparts->getPid() )!=321  ){ 
						...
						TLorentzVector parent_temp;
						parent_temp.SetXYZM(mcparts->getPx(), mcparts->getPy(), mcparts->getPz(), mcparts->getMass());
						if(genPi_dummy.getZ() > .3) good_sim_pi = true;

						if( good_dat_pi || good_sim_pi ){
							parent_p.push_back(parent_temp);
							pi_dummy.setParent( mcparts->getPid() );
							pi_gen.push_back(genPi_dummy); 
						}
					}
				}
				if( good_dat_pi || good_sim_pi ){ pi.push_back(pi_dummy); }
