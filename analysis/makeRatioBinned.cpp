...
int main(int argc, char** argv) {
    ...
    cerr << "  Rho File        : " << rho_file_name << "\n";
    cerr << "  Rho Norm File   : " << rho_norm_name << "\n";
	...
	TFile*     rhoFiles[N_E]     = {nullptr, nullptr, nullptr};
	TH3F*      hNorms_pip[N_E]   = {nullptr, nullptr, nullptr};
	TH3F*      hNorms_pim[N_E]   = {nullptr, nullptr, nullptr};
	TVector3*  bounds[N_E]       = {nullptr, nullptr, nullptr};
	double     E_list[N_E]       = {10.2, 10.4, 10.6};  // or fill later

	double Mx_min = 0;//bounds->X();
	double Mx_max = 0; //bounds->Y();
	...
	if( inBeam == 0 || inBeam == 10.2){
		...
		if( applyCorr > 4 ){
			...
			getNewRhoName(10.2, rho_norm_name);
			cout<<rho_norm_name<<std::endl;
			TFile* rho_norms = new TFile((TString) _DATA + (TString)"/correctionFiles/" + rho_norm_name);
			rhoFiles[0]=rho_norms;
			hNorms_pip[0]=(TH3F*)rho_norms->Get("hNorm_pip");
			hNorms_pim[0]=(TH3F*)rho_norms->Get("hNorm_pim");
			bounds[0]=(TVector3*)rho_norms->Get("bounds");
	...
	if( inBeam == 0 || inBeam == 10.4){
		...
		if( applyCorr > 4 ){
			...
			getNewRhoName(10.4, rho_norm_name);
			cout<<rho_norm_name<<std::endl;
			TFile* rho_norms = new TFile((TString) _DATA + (TString)"/correctionFiles/" + rho_norm_name);
			rhoFiles[1]=rho_norms;
			hNorms_pip[1]=(TH3F*)rho_norms->Get("hNorm_pip");
			hNorms_pim[1]=(TH3F*)rho_norms->Get("hNorm_pim");
			bounds[1]=(TVector3*)rho_norms->Get("bounds");
	...
	if( inBeam == 0 || inBeam == 10.6){
		...
		if( applyCorr > 4 ){
			...
			getNewRhoName(10.6, rho_norm_name);
			cout<<rho_norm_name<<std::endl;
			TFile* rho_norms = new TFile((TString) _DATA + (TString)"/correctionFiles/" + rho_norm_name);
			rhoFiles[2]=rho_norms;
			hNorms_pip[2]=(TH3F*)rho_norms->Get("hNorm_pip");
			hNorms_pim[2]=(TH3F*)rho_norms->Get("hNorm_pim");
			bounds[2]=(TVector3*)rho_norms->Get("bounds");
	...
	if( applyCorr > 4 ){
		while (reader_r.Next()) {
			...
			double Mx_min = bounds[this_bin_E]->X();
			double Mx_max = bounds[this_bin_E]->Y();

			if( (*Mx_2pi)*(*Mx_2pi) > 1.25 ) continue;
			...
			for( int E = 0; E < 3; E++ ){	
				...
				for( int k = 0; k < bins_Z; k++ ){
					...
					if( applyCorr <= 3){
						...
						if ( !hNorms_pip[E] )continue;
						double norm_pip = hNorms_pip[E]->GetBinContent(j, i, k+1 );
						double norm_pim = hNorms_pim[E]->GetBinContent(j, i, k+1 );

						setBin( hZ_r_bac[i-1][j-1][0],  k, events_in_bin[3][0][i-1][j-1], errors_in_bin[3][0][i-1][j-1],weights_in_bin[3][0][i-1][j-1][k][E], corrector, 4, matchType, 0, E, norm_pip );
						setBin( hZ_r_bac[i-1][j-1][1],  k, events_in_bin[3][1][i-1][j-1], errors_in_bin[3][1][i-1][j-1],weights_in_bin[3][1][i-1][j-1][k][E], corrector, 4, matchType, 1, E, norm_pim );
