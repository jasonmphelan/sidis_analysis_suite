...
int main( int argc, char** argv){
	...
	while (reader_rec.Next()) {
		...
		for( int i = 0; i < (int) ( pi.end() - pi.begin() ); i++ ){
			...
			sumWeights[0][this_bin_Q2][this_bin_xB][this_bin_Z][chargeIdx] += weight*weight;	
			sumWeightsErr[0][this_bin_Q2][this_bin_xB][this_bin_Z][chargeIdx] += eventWeightErr*eventWeightErr;	
	...
	TFile * rho_norms = new TFile("/work/clas12/users/jphelan/sidis_analysis_suite/data/correctionFiles/rho_norms_no_acc.root");
