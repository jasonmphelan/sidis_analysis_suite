...
int main( int argc, char** argv){
	...
	TFile * outFile = new TFile("outZ.root", "RECREATE");
	...
	for( int i = 1; i <= bins_Q2; i++ ){
		for( int j = 1; j <= bins_xB; j++ ){
			...
			//cout<<hZ_r[i-1][j-1][1]->Integral()<<std::endl;
			...
			hZ[i-1][j-1][0]->Draw();
			hZ[i-1][j-1][1]->SetLineColor(kBlue);
			hZ[i-1][j-1][1]->Draw("SAME");
			
			hZ_k[i-1][j-1][0]->SetLineColor(kRed);
			hZ_k[i-1][j-1][0]->Draw("SAME");
			hZ_k[i-1][j-1][1]->SetLineColor(kOrange);
			hZ_k[i-1][j-1][1]->Draw("SAME");
			hZ_r[i-1][j-1][0]->SetLineColor(kGreen);
			hZ_r[i-1][j-1][0]->Draw("SAME");
			hZ_r[i-1][j-1][1]->SetLineColor(kMagenta);
			hZ_r[i-1][j-1][1]->Draw("SAME");
			//hZ_r_bac[i-1][j-1][1]->SetLineColor(kRed);
			//hZ_r_bac[i-1][j-1][1]->Draw();
			//hZ_r_bac[i-1][j-1][0]->SetLineColor(kOrange);
			//hZ_r_bac[i-1][j-1][0]->Draw("SAME");
			/*
			hZ[i-1][j-1][0]->Add(hZ_k[i-1][j-1][0]);
			hZ[i-1][j-1][0]->Add(hZ_r[i-1][j-1][0]);
			hZ[i-1][j-1][0]->Add(hZ_r_bac[i-1][j-1][0], -1);
			hZ[i-1][j-1][0]->SetLineColor(kBlack);
			hZ[i-1][j-1][0]->Draw("SAME");
			...
			*/
			...
			hZ_k[i-1][j-1][0]->SetDirectory(outFile);
			hZ_k[i-1][j-1][1]->SetDirectory(outFile);
			hZ_r[i-1][j-1][0]->SetDirectory(outFile);
			hZ_r[i-1][j-1][1]->SetDirectory(outFile);
