...
for q in range(12):
	...
	ax[1].set_ylabel(rf'$r(z, x_B)/$FF($z$))', fontsize=18)
	ax[0].set_ylabel(r'$r(z)$', fontsize=18)

	ax[0].text(.7, .7, r'%.1f $< Q^2 <$ %.1f'%(2 + .5*(q), 2+.5*(q+1)), fontsize=18)
	rat_max = 1.05
	rat_min = 0.95
	for x in range(14):
		...
		binCenters = np.asarray((xEdges[:-1]+xEdges[1:])/2 + ( -.01 + .002*x))
		...
		ax[0].errorbar( binCenters, values, errors, label=label_x, marker='o',
						color = colorList[x], linestyle = '',capsize = 2, lw = 1, capthick = 1, mec='black')
	...
	fig.savefig('{0}/ratio_{1}.png'.format(outFileName, q+1))
