...
for x in range(14):
	...
	for q in range(12):
		...
		binCenters = np.asarray((xEdges[:-1]+xEdges[1:])/2 + ( -.01 + .002*(q-6)))

		ax[0].errorbar( binCenters, values, errors, label=r'%.1f $< Q^2 <$ %.1f'%(2 + .5*(q), 2+.5*(q+1)), marker='o',
						color = colorList[q], linestyle = '',capsize = 2, lw = 1, capthick = 1, mec='black')
	...
	fig.savefig('{0}/ratio_{1}.png'.format(outFileName, x+1))
