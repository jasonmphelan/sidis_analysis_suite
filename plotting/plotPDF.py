...
def load_file(path):
	...
	# When makeRatioBinned3D ran with applyCorr > 3 it uses setCombinedBin, so
	# hRatio already contains the fully combined corrected yield (pi + kaon - rho).
	# Adding hRatio_k / subtracting hRatio_r on top would double-count.
	corr_combined_key = "corrections_combined;1"
	if corr_combined_key not in inFile and "corrections_combined" in inFile:
		corr_combined_key = "corrections_combined"
	corrections_already_combined = (
		corr_combined_key in inFile and
		int(inFile[corr_combined_key].value) == 1
	)
	...
	for var in range(n_var):
		for q in range(BINS_Q2):
			for x in range(BINS_XB):
				...
				# Skipped when corrections_combined==1 because hRatio already
				# includes the kaon (and rho) correction from setCombinedBin.
				if APPLY_CORR > 3 and not corrections_already_combined:
				...
				if APPLY_CORR > 4 and not corrections_already_combined:
