...
//	file->Add("/volatile/clas12/users/jphelan/SIDIS/data/final_skims/10.6/final_skim.root");
	...
	while (reader.Next()) {
		...
		for( int i = 0; i < (int) ( pi.end() - pi.begin() ); i++ ){
			...
			if( pi[i].get3Momentum().Mag() < p_bin_edges[p_bin] ){ continue; }
			if( pi[i].get3Momentum().Mag() > p_bin_edges[p_bin+1] ){ continue; }
		...
		for( int q_bin = 0; q_bin < 500; q_bin++ ){
			for( int z_bin = 0; z_bin < 500; z_bin++ ){
				for( int charge = 0; charge < 2; charge++ ){
					...
					corr.setKinematics( x_val, q2_val, z_val, (p_bin_edges[p_bin] + p_bin_edges[p_bin+1])/2. );
					double correction = corr.getCorrectionFactor( 2, charge);
