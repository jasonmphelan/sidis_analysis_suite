...
int main( int argc, char** argv){
	...
	if(type == 0){
		...
		if( target == 1 ){
			file->Add("../trees/final_skims/10.2/final_skim_rga.root");
		}
	}
	if(type == 1){
		...
		if( target == 1 ){
			file->Add("../trees/final_skims/kaons_10.2/final_skim_rga.root");
		}
	}
	...
	while (reader.Next()) {
		...
		for( int i = 0; i < (int) ( pi.end() - pi.begin() ); i++ ){
			if( (theta_cut==0 && !isGoodPion_no_acc_vec[i]) || (theta_cut>0 && !isGoodPion_vec[i]) ){continue;}
